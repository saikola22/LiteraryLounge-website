...
function Article() {
  ...
  const deleteArticle = async() => {
    ...
    delete art._id;
    let res=await axiosWithToken.put(`http://localhost:4000/author-api/book/${currentArticle.articleId}`,art)
    if(res.data.message==='article deleted'){
      setCurrentArticle({...currentArticle,status:res.data.payload})
    }
  };
  // <img src={currentArticle.link} alt="image" height="100" width="100"/>

  const restoreArticle =async () => {
    let art={...currentArticle};
    delete art._id;
    let res=await axiosWithToken.put(`http://localhost:4000/author-api/book/${currentArticle.bookId}`,art)
  ...
  const saveModifiedArticle = async (editedArticle) => {
    ...
    let res = await axiosWithToken.put(
      "http://localhost:4000/author-api/book",
      modifiedArticle
    );
    if (res.data.message === "Article modified") {
      setArticleEditStatus(false);
      navigate(`/author-profile/book/${modifiedArticle.bookId}`, {
  ...
  <div className="col-sm-12 col-md-6 p-4">
    <img src={state.link}  alt="Responsive" height="500px" width="350px"/>
  </div>
